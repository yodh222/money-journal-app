              ...
              <tbody>
                {transactions && transactions.length > 0 ? (
                  transactions.map((tx: any) => (
                    <tr key={tx.id} className="border-b border-[#27272A] last:border-0 hover:bg-[#27272A]/20 transition-colors">
                      <td className="px-6 py-4 text-zinc-300">
                        {new Date(tx.transaction_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-[#27272A] text-zinc-300">
                          {tx.categories?.name || 'Tanpa Kategori'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-zinc-300">{tx.notes || '-'}</td>
                      <td className={`px-6 py-4 text-right font-bold ${tx.categories?.type === 'INCOME' ? 'text-emerald-400' : 'text-zinc-200'}`}>
